...
case class Row(text: String, delimiter: Char, numColumns: Option[Int] = None){
  ...
  private def hasTooManyEntries: Option[RowStatus] =
    if (text.count(_ == delimiter) + 1 > numColumns.getOrElse(0)) Some(TooManyColumns) else None

  private def hasTooFewEntries: Option[RowStatus] =
    if (text.count(_ == delimiter) + 1 < numColumns.getOrElse(0)) Some(TooFewColumns) else None
